...
const child = spawn(process.execPath, [serverPath], { stdio: ["pipe", "pipe", "inherit"], env: { ...process.env, API_FORGE_DATA: testData, API_FORGE_PORT: "43129", AIPI_REGISTRY_FILE: registryFile } });
...
try {
  ...
  assert.ok(["trace_route", "diff_contract", "run_local_diagnostic", "generate_fixture", "check_blast_radius"].every((name) => listed.result.tools.some((tool) => tool.name === name)));
  ...
  assert.equal(scanned.result.structuredContent.scan.endpoints.length, 2);
  ...
  assert.equal(traced.result.structuredContent.evidence.contract.status, "drift");
  ...
  assert.ok(summary.result.structuredContent.summary.corrections.schema.length > 0);
