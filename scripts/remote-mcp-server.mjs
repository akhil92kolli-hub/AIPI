...
const tools = [
  ...
  { name: "list_contract_versions", title: "List contract versions", description: "List immutable contract publications for one repository route when the configured registry supports version history.", inputSchema: { type: "object", required: ["organizationId"], properties: { organizationId: { type: "string" }, repository: { type: "string" }, method: { type: "string" }, route: { type: "string" } }, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "list_registry_audit", title: "List registry audit events", description: "List recent immutable contract publication audit events when supported by the configured registry.", inputSchema: { type: "object", required: ["organizationId"], properties: { organizationId: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 500, default: 100 } }, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } }
];
...
async function dispatch(message) {
  ...
  if (message.method === "tools/call") {
    ...
    if (name === "check_blast_radius") { const report = await checkBlastRadius(registry, args); return result({ report }, report.status === "unverified" ? report.note : report.safe ? "No registered consumer breakages detected." : report.impacts.map((entry) => entry.message).join("\n")); }
    ...
    if (name === "list_contract_versions") { const versions = registry.listVersions ? await registry.listVersions(args) : []; return result({ versions, supported: Boolean(registry.listVersions) }, registry.listVersions ? `${versions.length} immutable contract version(s) found.` : "The configured registry does not expose version history yet."); }
    if (name === "list_registry_audit") { const events = registry.listAudit ? await registry.listAudit(args.organizationId, args.limit) : []; return result({ events, supported: Boolean(registry.listAudit) }, registry.listAudit ? `${events.length} registry audit event(s) found.` : "The configured registry does not expose audit history yet."); }
